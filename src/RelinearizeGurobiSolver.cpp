...
ProblemResult RelinearizeGurobiSolver::solve(const int kr, const int km, const CircuitGraph &graph, const int max_threads){
  timer.set();
  ...
  GRBLinExpr obj = 0;
  {
    ...
    for (int i=0; i<graph.n_gate; i++) if(graph.gates[i].type == AND) {
      sum_mul += l[i] + x[i];
    }
  ...
  for (int i=0; i<graph.n_gate; i++) {
    ...
    if (graph.gates[i].type == AND) {
      ...
      if (p1 != -1) model.addConstr(l[i] >= l[p1] - x[i]);
      if (p2 != -1) model.addConstr(l[i] >= l[p2] - x[i]);
