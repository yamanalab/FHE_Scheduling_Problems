...
CircuitGraph::CircuitGraph(const std::string &filename){
  ...
  for (int i = 0; i < n_gate; ++i) {
    ...
    if (n_in == 2) assert(0 <= in2 && in2 <= n_wire);
    ...
    if(in1 >= 0) wires[in1].dests.push_back(i);
    if(in2 >= 0) wires[in2].dests.push_back(i);
  ...
  for (int i=0; i<n_input; i++) assert(wires[i].src == -1);
}
...
CircuitGraph CircuitGraph::reduceSize(){
  ...
  while(!que.empty()){
    int i = que.front(); que.pop(); // edge(wire) index
    for(auto &gi : wires[i].dests) {
      if (gates[gi].type == AND) continue;
      bool toNext = false;
      if(num_v[gi] == -1){
        // first visit of the node
        num_v[gi] = num_e[i];
        if (gates[gi].type == INV) toNext = true;
      } else {
  ...
  std::set<int> out_wires, general_wires;
  {
    ...
    for(int i = n_input; i < n_wire - n_output; i++) {
      if(num_e[i] >= n_input && out_wires.find(num_e[i]) == out_wires.end()) general_wires.insert(num_e[i]);
    }

    int cnt = 0;
    for(int i=0; i<n_input; i++) nume2new_idx[i] = cnt++;
    for(auto i : general_wires) nume2new_idx[i] = cnt++;
    for(auto i : out_wires) nume2new_idx[i] = cnt++;
    nw.resize(cnt);

    cnt = 0;
    for(int i = 0; i < n_gate; i++) {
      if(numv2new_idx.find(num_v[i]) == numv2new_idx.end()) numv2new_idx[num_v[i]] = cnt++;
    }
    ng.resize(cnt);

    for(int i=0; i<n_gate; i++){
      if((gates[i].type == XOR && gates[i].in1 == gates[i].in2 && gates[i].in1 != -1 && num_e[gates[i].in1] < n_input)
        || (gates[i].type == INV && num_e[gates[i].in1] < n_input)) {
        input_vertices[num_e[gates[i].in1]] = numv2new_idx[num_v[i]];
      }
    }

    for(int i=0; i<n_wire; i++) num_e[i] = nume2new_idx[num_e[i]];
    for(int i=0; i<n_gate; i++) num_v[i] = numv2new_idx[num_v[i]];
  }
  ...
  for(int i=0; i<n_wire; i++) {
    assert(ng[num_v[wires[i].src]].out == -1 || ng[num_v[wires[i].src]].out == num_e[i]);
    ng[num_v[wires[i].src]].out = num_e[i];
    assert(nw[num_e[i]].src == -1 || nw[num_e[i]].src == num_v[wires[i].src]);
  ...
  for(int i=0; i<n_input; i++){
    wires[i].src = -1;
    nw[num_e[i]].src = -1;
    if(i < trick_add_ng_cnt) ng.pop_back();
  }
