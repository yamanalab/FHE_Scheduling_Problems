...
enum gate_type {XOR, AND, INV, IN, OUT};
...
public:
  ...
  gate_type type;
  Gate(int _in1, int _in2, int _out, gate_type _type) : in1(_in1), in2(_in2), out(_out), type(_type) {};
  Gate() {};
};
...
private:
  CircuitGraph();
  CircuitGraph(const std::vector<Gate> &ng, const std::vector<Wire> &nw, const int nin, const int nout)
   : n_gate(ng.size()), n_wire(nw.size()), n_input(nin), n_output(nout), gates(ng), wires(nw) {};
