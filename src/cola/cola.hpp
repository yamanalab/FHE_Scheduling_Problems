...
public:
    ...
    /**
     *  Returns passed argument. if T is different from the type of defined one,
     * throw exception.
    ...
    /**
     *  Returns passed argument as vector. if T is different from the type of
     * defined one, throw exception.
     *  @tparam T type of argument. integer, real, string is allowed.
     */
    template <typename T>
...
// ctor/dtor
// ---------------------------------------------------------------------------------
inline parser::parser() : parsed_(false), exists_undefined_(false)
{
}
...
// define
// ------------------------------------------------------------------------------------
inline parser::option& parser::define(const string& name,
                                      const string& description)
{
    ...
    if (is_defined(name))
    {
        throw logic_error("the option '" + make_arg(name) +
                          "' is already defined");
    }
    ...
    const string desc = description.empty()
                          ? "option #" + to_string(options_.size() + 1)
                          : description;
...
inline string parser::descriptions(const string& offset) const
{
    ...
    for (; it != options_.end(); ++it)
    {
        ...
        if (names.size() >= 2)
            oss << " (";
        for (svector::const_iterator it = names.begin() + 1; it != names.end();
             ++it)
        {
...
inline parser::option& parser::get_option(const string& name)
{
    ...
    if (it == lookup_.end())
        throw logic_error("an option '" + arg + "' is undefined");
    return *it->second;
}
...
// eval each arguments
// ------------------------------------------------------------------------
inline void parser::eval_short_option(svector::iterator& it,
                                      svector::iterator end)
{
...
inline void parser::eval_shorts_option(svector::iterator& it, svector::iterator)
{
    ...
    for (svector::size_type j = 1; j < arg.size(); j++)
    {
        ...
        if (opt.has_arg())
        {
            throw runtime_error("the option '" + make_arg(c) +
                                "' needs argument");
        }
        opt.passed();
    }
...
inline void parser::eval_long_option(svector::iterator& it,
                                     svector::iterator end)
{
    const string& arg = *it++; // it points next of arg "--abc"
...
inline void parser::eval_long_param_option(svector::iterator& it,
                                           svector::iterator)
{
...
inline string parser::to_string<string>(const std::vector<string>& value)
{
    std::stringstream ss;
    ss << "[";
    for (std::vector<string>::const_iterator it = value.begin();
         it != value.end(); ++it)
    {
        ss << '"' + *it + '"' << ((it == value.end() - 1) ? "" : ", ");
    }
...
inline T parser::from_string(const string& value)
{
    ...
    {
        throw runtime_error("cast failed: '" + value + "' to " +
                            to_string<T>());
    }
    return ret;
}
...
inline parser::option& parser::option::with_arg<integer>(integer default_value)
{
    ...
    if (type_ != FLAG)
        throw logic_error("parser::option::with_arg() cal be called only once");
...
// -----------------------------------------------------------------------------------
template <>
inline parser::option& parser::option::with_arg_vector<integer>(
  const std::vector<integer>& default_value)
{
...
template <>
inline parser::option& parser::option::with_arg_vector<real>(
  const std::vector<real>& default_value)
{
    mother_.must_not_be_parsed("parser::option::with_arg_vector");
...
template <>
inline parser::option& parser::option::with_arg_vector<string>(
...
// ctor
// -----------------------------------------------------------------------------------
inline parser::option::option(const string& name, const string& description,
                              parser& mother)
  : mother_(mother),
    type_(FLAG),
    value_s_(NULL),
...
inline parser::option::option(char name, const string& description,
                              parser& mother)
  : mother_(mother),
    type_(FLAG),
    value_s_(NULL),
...
inline integer parser::option::as<integer>() const
{
    ...
    if (type_ != INTEGER)
        throw logic_error("argument type of '" + names_[0] +
                          "' is not integer");
    return value_i_;
}
...
inline real parser::option::as<real>() const
{
    ...
    if (type_ != REAL)
        throw logic_error("argument type of '" + names_[0] + "' is not real");
    return value_r_;
}
    ...
    switch (type_)
    {
        ...
        case SVECTOR:
            *value_sv_ = from_string<string>(arg, delim_);
            break;
        default:
            throw logic_error("the option '" + names_[0] +
